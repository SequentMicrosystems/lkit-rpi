...
#define COMM_INTERCHUNK_DELAY_US 200000
// SMBus messages

#define I2C_SMBUS_BLOCK_MAX	128	/* As specified in SMBus standard */
#define I2C_SMBUS_I2C_BLOCK_MAX	128	/* Not specified but we use same structure */
...
{
	if ( (NULL == buff) || (size > I2C_SMBUS_BLOCK_MAX))
	{
		return -1;
	}
...
int i2cWrite(int dev, uint8_t* buff, int size)
{
	...
	while (size > 0)
	{
		...
		if(size > 0)
		{
			usleep(COMM_INTERCHUNK_DELAY_US);		//uS (unsigned int)
		}
		buff+=chunk;
	}
	return 0;
...
int i2cMem8Write(int dev, int add, uint8_t* buff, int size)
{
	...
	if ( (NULL == buff) || (size > I2C_SMBUS_BLOCK_MAX - 1))
	{
		return -1;
	}
